...
function renderEstimatedDevices() {
  ...
  for (const device of state.deviceState.values()) {
    ...
    const iconSize = 8 + Math.min(18, Math.round(Math.log2(device.obsCount + 1) * 3));
    const icon = L.divIcon({
      className: '',
      html: `<div class="device-marker ${escapeHtml(device.type)} ${confidenceClass(device.confidenceLevel)}" style="width:${iconSize}px;height:${iconSize}px;background:${style.fill};border-color:${style.stroke};"></div>`,
      iconSize: [iconSize, iconSize],
      iconAnchor: [Math.ceil(iconSize / 2), Math.ceil(iconSize / 2)],
    });

    const marker = L.marker([device.estLat, device.estLon], { icon, keyboard: false });
    marker.bindPopup(renderDevicePopup(device));
    marker.bindTooltip(renderDeviceTooltip(device), { direction: 'top', offset: [0, -8] });
    marker.addTo(estimatedLayer);
  }
}

function renderDeviceTooltip(device) {
  const confidence = ['Low', 'Medium', 'High'][device.confidenceLevel];
  const displayName = device.ssid || device.mac;
  const typeLabel = device.type.charAt(0).toUpperCase() + device.type.slice(1);
  return `${escapeHtml(typeLabel)} · ${escapeHtml(displayName)} · ${confidence} · ${device.obsCount} obs`;
}
...
function styleForConfidence(level) {
  if (level === 2) return { stroke: '#10b981', fill: '#34d399' };
  if (level === 1) return { stroke: '#f59e0b', fill: '#fbbf24' };
